...
namespace ds::utils
{
    ...
    {
        std::vector<size_t> velkosti;
        for (size_t i = 0; i < getStepCount(); i++)
        {
            velkosti.push_back((i + 1) * getStepSize());
        }

        using vector_replication_t = std::vector<duration_t>;
        std::vector<vector_replication_t> results;
        results.reserve(getReplicationCount());

        for (size_t replication = 0; replication < getReplicationCount(); replication++)
        {
            Structure testStructure(structurePrototype);

            vector_replication_t repResults;
            for (size_t i = 0; i < getStepCount(); i++)
            {
                size_t expectedSize = velkosti[i];
                growToSize(testStructure, expectedSize);
                ...
                repResults.push_back(duration);
            }
            results.push_back(repResults);
        }

        saveToCsvFile(velkosti, results);
    }
