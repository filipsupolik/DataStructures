...
namespace ds::adt {
    ...
    void GeneralBinarySearchTree<K, T, ItemType>::insert(const K& key, T data)
    {
        ...
        else
        {
            ...
            {
                throw structure_error("GeneralBinaryTree<K, T, ItemType>::insert: key already there!");
            }
    ...
    T GeneralBinarySearchTree<K, T, ItemType>::remove(const K& key)
    {
        ...
        if (!this->tryFindNodeWithKey(key, node))
        {
            throw structure_error("GeneralBinaryTree<K, T, ItemType>::remove: key not there!");
        }
